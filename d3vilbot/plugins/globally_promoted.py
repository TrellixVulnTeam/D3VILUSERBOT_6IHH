...
@d3vil_cmd(pattern="gpro(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    d3vil = await eor(event, "`Promoting globally...`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await d3vil.edit("You can't promote yourself...")
       return
    try:
        if not rank:
            rank = "ㅤ"
    except:
        return await d3vil.edit("**ERROR !!**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await d3vil.edit(f"**Promoting User in :**  `{i}` Chats...")
          except:
             pass
    else:
        await d3vil.edit(f"**Reply to a user !!**")
    await d3vil.edit(
...
@d3vil_cmd(pattern="gdem(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    d3vil = await eor(event, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await event.get_chat()
    if event.is_private:
    ...
    try:
        ...
        pass
    if me == user:
       k = await d3vil.edit("You can't Demote yourself !!")
       return
    try:
        if not rank:
            rank = "ㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await event.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await d3vil.edit(f"**Demoting Globally In Chats :** `{i}`")
          except:
             pass
    else:
        await d3vil.edit(f"**Reply to a user !!**")
...
@d3vil_cmd(pattern="gban(?:\s|$)([\s\S]*)")
async def _(event):
    d3vil = await eor(event, f"`Gbanning ...`")
    reason = ""
    cid = await client_id(event)
    d3vilkrish, D3VIL_USER, d3vil_mention = cid[0], cid[1], cid[2]
...
@d3vil_cmd(pattern="ungban(?:\s|$)([\s\S]*)")
async def _(event):
    cid = await client_id(event)
    d3vilkrish, D3VIL_USER, d3vil_mention = cid[0], cid[1], cid[2]
    ...
    await d3vil.edit(f"Ungbaning in client 1... \n**Unbanned in :** `{chats}`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            d3vil = int(hel)
            try:
                tity = await event.client.get_entity(d3vil)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={d3vil}) (`{d3vil}`)\n"
    else:
...
@d3vil_cmd(pattern="gkick(?:\s|$)([\s\S]*)")
async def gkick(event):
    cid = await client_id(event)
    d3vilkrish, D3VIL_USER, d3vil_mention = cid[0], cid[1], cid[2]
    d3vil = await eor(event, f"`Kicking globally ...`")
    reply = await event.get_reply_message()
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
...
@d3vil_cmd(pattern="gmute(?:\s|$)([\s\S]*)")
async def gm(event):
    cid = await client_id(event)
    d3vilkrish, D3VIL_USER, d3vil_mention = cid[0], cid[1], cid[2]
    d3vil = await eor(event, "`Trying to gmute user...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        usr = event.text.split(" ", maxsplit=2)[1]
        ...
        return await eod(d3vil, "**To gmute a user I'll need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    reply = await event.get_reply_message()
    if gsql.is_gmuted(userid, "gmute"):
        return await eod(d3vil, "This kid is already Gmuted.")
    if str(userid) in DEVLIST:
        return await eod(d3vil, "**Sorry I'm not going to gmute them..**")
    try:
        ...
        if Config.ABUSE == "ON":
            ...
            await eor(d3vil, f"**Globally Muted [{name}](tg://user?id={userid}) !!**\n\n__Kid struggling to speak__ ♪～(´ε｀ )")
    except Exception as e:
        await eod(d3vil, "Error occured!\nError is " + str(e))


@d3vil_cmd(pattern="ungmute(?:\s|$)([\s\S]*)")
async def endgmute(event):
    d3vil = await eor(event, "`Trying to ungmute !!`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        usr = event.text.split(" ", maxsplit=2)[1]
        ...
        return await eod(d3vil, "**To ungmute a user I'll need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    reply = await event.get_reply_message()
    if not gsql.is_gmuted(userid, "gmute"):
        return await eod(d3vil, "I don't remember I gmuted him/her...")
    try:
        gsql.ungmute(userid, "gmute")
        await eor(d3vil, f"**Unmuted [{name}](tg://user?id={userid}) Globally !!**")
    except Exception as e:
        await eod(d3vil, "Error occured!\nError is " + str(e))
