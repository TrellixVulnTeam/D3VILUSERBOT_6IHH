...
@d3vil _cmd(pattern="trigger$")
async def _(event):
    d3vil  = await eor(event, "`Trigggggggggerrr`")
...
@d3vil_cmd(pattern="geyy$")
async def _(event):
    d3vil = await eor(event, "`Geyyyy`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://telegra.ph{url_s[0]}"
...
@d3vil_cmd(pattern="pix$")
async def _(event):
    d3vil = await eor(event, "`Pixing This Image.`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://telegra.ph{url_s[0]}"
