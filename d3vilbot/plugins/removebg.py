...
async def _(event):
    ...
    d3vil _memtion = cid[2]
    if event.reply_to_msg_id and input_str == "":
        reply_message = await event.get_reply_message()
        d3vil  = await eor(event, "`Analysing...`")
        ...
        try:
            ...
            await d3vil .edit("`Removing background of this media`")
            file_name = toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        d3vil  = await eor(event, "`Removing Background of this media`")
    ...
    if "image" in contentType:
        ...
        await eod(d3vil , f"`{response.content.decode('UTF-8')}`")
        return
    if flag and flag == "-s":
        file = tosticker(remove_bg_image, filename="D3vilBot.webp")
    ...
    await event.client.send_file(
        event.chat_id,
        file,
        caption=f"**Background removed by** {d3vil _memtion}",
        force_document=True,
        )
    await d3vil.delete()
