...
async def get_media(event):
    ...
    d3vil = await eor(event, "Downloading Media From this Channel.")
    ...
    await d3vil.edit("Downloaded " + output + " files.")


@d3vil_cmd(pattern="geta ([\s\S]*)")
async def get_media(event):
    dir = "./channel_dwl/"
    try:
        os.makedirs("./channel_dwl/")
    ...
    print(channel_username)
    d3vil = await eor(event, "Downloading All Media From this Channel.")
...
CmdHelp("chnl_dwl").add_command(
  ...
  "getc", "<limit> <channel username>", "will download latest given number of media from channel into your bot server", "getc 1000 @D3VIL_BOT_OFFICIAL"
).add_info(
