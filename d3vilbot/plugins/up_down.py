...
@d3vil_cmd(pattern="upld_dir(?:\s|$)([\s\S]*)")
async def uploadir(event):
    input_str = event.pattern_match.group(1)
    if os.path.exists(input_str):
        d3vil = await eor(event, "Downloading Using Userbot Server....")
        lst_of_files = []
        ...
        uploaded = 0
        await d3vil.edit("Found {} files. Uploading will start soon. Please wait!".format(len(lst_of_files)))
        for single_file in lst_of_files:
            if os.path.exists(single_file):
                ...
                uploaded = uploaded + 1
        await d3vil.edit("Uploaded {} files successfully !!".format(uploaded))
    else:
        await d3vil.edit("404: Directory Not Found")

@d3vil_cmd(pattern="upload(?:\s|$)([\s\S]*)")
async def upload(event):
    ...
    if os.path.exists(input_str):
        ...
        await d3vil.edit("Uploaded successfully !!")
    else:
        await d3vil.edit("404: File Not Found")
...
@d3vil_cmd(pattern="upld_as_(stm|vnr|all)(?:\s|$)([\s\S]*)")
    ...
    if os.path.exists(file_name):
        ...
        try:
            if supports_streaming:
                ...
                await d3vil.edit("TBD: Not (yet) Implemented")
                return
            os.remove(thumb)
            await d3vil.edit("Uploaded successfully !!")
        except FileNotFoundError as err:
            await d3vil.edit(str(err))
    else:
        await d3vil.edit("404: File Not Found")

@d3vil_cmd(pattern="download(?:\s|$)([\s\S]*)")
async def _(event):
    d3vil = await eor(event, "`Processing ...`")
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            downloaded_file_name = await event.client.download_media(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, d3vil, c_time, "trying to download")
                ),
            )
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff % 10.00) == 0 and current_message != display_message:
                    await d3vil.edit(current_message)
        ...
        if downloader.isSuccessful():
            ...
            await d3vil.edit("Incorrect URL\n {}".format(input_str))
    else:
        await d3vil.edit("Reply to a message to download to my local server.")
