...
@d3vil_cmd(pattern="commit(?:\s|$)([\s\S]*)")
async def download(event):
    ...
    try:
        ...
        await git_commit(downloaded_file_name, path, branch, d3vilbot)


async def git_commit(file_name, path, branch, d3vilbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await d3vilbot.edit("`File Already Exists`")
            create_file = False
    path = path
    file_name = file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(d3vilbot, "Cannot Upload File")
    else:
        return await eod(d3vilbot, "`Committed Suicide`")


@d3vil_cmd(pattern="github(?:\s|$)([\s\S]*)")
async def _(event):
...
CmdHelp("github").add_command(
  "commit", "<reply to a file> <path>|<branch>", "Uploads the file on github repo as provided in Heroku Config GIT_REPO_NAME. In short makes a commit to git repo from Userbot", "commit ./d3vilbot/plugins/example.py|master"
).add_command(
