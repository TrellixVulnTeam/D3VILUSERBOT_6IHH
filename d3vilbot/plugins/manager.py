...
async def _(event):
    ...
    OUTPUT = f"**Files in ᗪ3ᏉᎥᏝᏰᎧᏖ DOWNLOADS Folder:**\n"
...
@d3vil_cmd(pattern="rnsaved(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@d3vil_cmd(pattern="rnlocal(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@d3vil_cmd(pattern="delsave(?:\s|$)([\s\S]*)")
...
@d3vil_cmd(pattern="delocal(?:\s|$)([\s\S]*)")
async def handler(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("managers").add_command(
  ...
  "ls_root", None, "Gives the list of all files in root directory of d3vilbot repo."
).add_command(
