...
@bot.on(admin_cmd(pattern=f"yt(a|v) ?(.*)", outgoing=True))
...
@d3vilbot.on(d3vil_cmd(outgoing=True, pattern="ytlink$"))
async def hmm(event):
    query = event.text[8:]
    if query == "":
    ...
    try:
        results = json.loads(D3vil_YTS(query, max_results=7).to_json())
    except KeyError:
