...
if PM_ON_OFF != "DISABLE":
    ...
    @D1.on(admin_cmd(pattern="(a|approve|allow)$"))
    ...
    @D1.on(admin_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @D1.on(admin_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
    ...
    async def on_new_private_message(event):
        if not event.is_private:
            return
        cid = await client_id(event)
        d3vilkrish, d3vil_mention = cid[0], cid[2]
        ...
        if Config.LOGGER_ID is None:
            await event.client.send_message(d3vilkrish, "Please Set `LOGGER_ID` For Working Of Pm Permit")
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        d3vilbot_ = await tap[0].click(event.chat_id)
        PM_WARNS[chat_ids] += 1
        if chat_ids in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_ids].delete()
        PREV_REPLY_MESSAGE[chat_ids] = d3vilbot_
...
CmdHelp("pm_permit").add_command(
  ...
  "listapproved", None, "Sends the list of all users approved by ᗪ3ᏉᎥᏝᏰᎧᏖ"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
