...
@d3vil_cmd(pattern="fpic$")
async def _(event):
    cid = await client_id(event)
    d3vil_mention = cid[2]
    url = "https://thispersondoesnotexist.com/image"
    response = requests.get(url)
    d3vil = await eor(event, "`Creating a fake face...`")
    if response.status_code == 200:
      with open("D3VILBOT.jpg", 'wb') as f:
        f.write(response.content)
    else:
        return await eod(d3vil, "Failed to create Fake Face! Try again later.")
...
@d3vil_cmd(pattern="gbam$")
async def gbun(event):
    gbunVar = event.text
    gbunVar = gbunVar[6:]
    mentions = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\n`"
    no_reason = "**Reason:**  __Madarchod Saala__"
    d3vil = await eor(event, "** Nikal Lawde❗️⚜️☠️**")
    asyncio.sleep(3.5)
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1676629806:
            await d3vil.edit("`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [D3VIL_KRISH](https://t.me/D3_krish) __to release your account__😏", link_preview=False)
        else:
            jnl = (
                "`Warning!! `"
                "[{}](tg://user?id={})"
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await d3vil.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Not Given `"
        await d3vil.edit(mention)
