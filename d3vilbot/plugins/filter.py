...
async def on_snip(event):
    ...
    name = event.raw_text
    _id = event.chat_id
    d3vilkrish, _, _ = await client_id(event)
    if not gvarstat(f"FILTER_{d3vilkrish}_{str(_id)[1:]}"):
        return
...
@d3vil_cmd(pattern="filter(?:\s|$)([\s\S]*)")
async def on_snip_save(event):
    name = event.pattern_match.group(1)
    msg = await event.get_reply_message()
    d3vilkrish, _, d3vil_mention = await client_id(event)
    _id = event.chat_id
    if msg:
...
@d3vil_cmd(pattern="rmallfilters$")
async def on_all_snip_delete(event):
    ...
    try:
        delgvar(f"FILTER_{d3vilkrish}_{str(_id)[1:]}")
