...
@d3vil_cmd(pattern="lock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[6:]
    cid = await client_id(event)
    d3vil_mention = cid[2]
    d3vil = await eor(event, f"Trying to lock `{text}`")
    if text == "":
        return await eod(d3vil, "Need something to lock...")
    locker = lock(text)
    if not locker:
        return await eod(d3vil, f"**🤐 Invalid lock type:** {text} \nDo `{hl}ltype` to get all lock types.")
    await event.client(EditChatDefaultBannedRightsRequest(event.chat_id, locker))
    await event.client.send_file(event.chat_id, restlo, caption=f"{d3vil_mention} Locked `{text}` \n__Cause its Rest Time !!__")
    await d3vil.delete()


@d3vil_cmd(pattern="unlock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[8:]
    cid = await client_id(event)
    d3vil_mention = cid[2]
    d3vil = await eor(event, f"Trying to unlock `{text}`")
    if text == "":
        return await eod(d3vil, "Need something to unlock...")
    unlocker = unlock(text)
    if not unlocker:
        return await eod(d3vil, f"**🤐 Invalid unlock type:** {text} \nDo `{hl}ltype` to get all unlock types.")
    await event.client(EditChatDefaultBannedRightsRequest(event.chat_id, unlocker))
    await event.client.send_file(event.chat_id, shuru, caption=f"**{d3vil_mention} unlocked** `{text}`")
    await d3vil.delete()


@d3vil_cmd(pattern="ltype$")
async def _(event):
