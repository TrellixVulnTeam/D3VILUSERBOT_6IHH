...
async def _(event):
    ...
    krishna = await eor(event, "Trying to download.... please wait!")
    async with event.client.conversation(bot) as conv:
        try:
            ...
            await krishna.edit("User Blocked!! Please Unblock @InstagramSaverRobot and try again...")
            return
    await krishna.delete()
    final = await event.client.send_file(event.chat_id, output_op)
    await final.edit(f"📥 InstaGram Video Downloaded By :- {d3vil_mention}")
