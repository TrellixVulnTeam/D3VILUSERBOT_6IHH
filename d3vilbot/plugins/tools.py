...
@d3vil_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
...
@d3vil_cmd(pattern="currency(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
    input_sgra = input_str.split(" ")
    if len(input_sgra) == 3:
...
@d3vil_cmd(pattern="unshort(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
