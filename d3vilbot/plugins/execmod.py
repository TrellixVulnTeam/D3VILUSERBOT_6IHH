...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**ᗪ3ᏉᎥᏝᏰᎧᏖ's Environment Module :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
@d3vil_cmd(pattern="speed$")
async def _(event):
    d3vil = await eor(event, "calculating...")
    PROCESS_RUN_TIME = 100
    cmd = "speedtest-cli"
    eply_to_id = event.message.id
    if event.reply_to_msg_id:
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**ᗪ3ᏉᎥᏝᏰᎧᏖ's Server Speed Calculated :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
CmdHelp("execmod").add_command(
  ...
  "env", None, "Shows Environment veriables of your D3vilBot"
).add_command(
  "speed", None, "Shows server speed of your D3vilBot"
).add_info(
  "Exec Modules."
).add_warning(
  "✅ Harmless Module."
