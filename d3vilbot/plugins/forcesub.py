...
from d3vilbot.sql.fsub_sql import *
from . import *


@D1.on(events.ChatAction())
...
async def _(event):
    ...
    if uid == event.sender_id:
        ...
        try:
            await D1(GetParticipantRequest(int(joinchat), uid))
        except Exception:
            await D2(GetParticipantRequest(int(joinchat), uid))
        except Exception:
            await D3(GetParticipantRequest(int(joinchat), uid))
        except Exception:
            await D4(GetParticipantRequest(int(joinchat), uid))
        except Exception:
            await D5(GetParticipantRequest(int(joinchat), uid))
        except Exception:
            ...
            return
        try:
            await D1.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
        except Exception:
            await D2.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
        except Exception:
            await D3.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
        except Exception:
            await D4.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
        except Exception:
            await D5.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
        except Exception:
