...
from d3vilbot.sql import antiflood_sql as sq
from . import *
...
@d3vilbot.on(admin_cmd(incoming=True))
async def _(event):
    if event.fwd_from:
        return
    if not CHAT_FLOOD:
...
@d3vilbot.on(admin_cmd(pattern="setflood(?: |$)(.*)"))
@d3vilbot.on(sudo_cmd(pattern="setflood(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
