...
async def _(event):
    ...
    if not gvarstat(f"WELCOME_{d3vilkrish }_{str(event.chat_id)[1:]}"):
        return
...
@d3vil_cmd(pattern="savewelcome(?:\s|$)([\s\S]*)")
async def save_welcome(event):
    ...
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{d3vilkrish }_{str(event.chat_id)[1:]}", "TRUE")
        return await eor(event, success.format("saved"))
    rm_welcome(event.chat_id)
    delgvar(f"WELCOME_{d3vilkrish }_{str(event.chat_id)[1:]}")
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{d3vilkrish }_{str(event.chat_id)[1:]}", "TRUE")
...
@d3vil_cmd(pattern="cleanwelcome$")
async def del_welcome(event):
    lg_id = Config.LOGGER_ID
    d3vilkrish , D3VIL_USER, d3vil_mention = await client_id(event)
    if gvarstat(f"WELCOME_{d3vilkrish }_{str(event.chat_id)[1:]}"):
...
@d3vil_cmd(pattern="welcome_note$")
