...
@d3vil_cmd(pattern="getpic(?:\s|$)([\s\S]*)")
async def _(event):
    d3vil = await eor(event, "Getting profile photo..")
    replied_user, error_i_a = await get_full_user(event)
    if replied_user is None:
        await d3vil.edit(str(error_i_a))
        return False
...
@d3vil_cmd(pattern="poto(?:\s|$)([\s\S]*)")
async def potocmd(event):
    uid = "".join(event.raw_text.split(maxsplit=1)[1:])
    user = await event.get_reply_message()
    chat = event.input_chat
    d3vil = await eor(event, "Getting profile pictures of this user...")
    if user:
    ...
    if uid.strip() == "":
        ...
        if int(uid) <= (len(photos)):
            ...
            await eod(d3vil, "No photo found of this NIBBA. Now u Die!")
            return
    elif uid.strip() == "all":
        if len(photos) > 0:
            ...
            try:
                ...
                await eod(d3vil, "**This user has no photos!**")
                return
    else:
        try:
            uid = int(uid)
            if uid <= 0:
                await eod(d3vil, "`Number Invalid!` **Are you komedy Me ?**")
                return
        except BaseException:
            await eod(d3vil, "Are you komedy me ?")
            return
        if int(uid) <= (len(photos)):
            send_photos = await event.client.download_media(photos[uid - 1])
            await event.client.send_file(event.chat_id, send_photos)
